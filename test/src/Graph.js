...
const Graph = ({ width, height }) => {
    ...
    const simulation = d3
        ...
        .force('x', d3.forceX().strength(xForceStrength))
        .force('y', d3.forceY().strength(yForceStrength))
        .force(
            'link',
            d3
    ...
    svg.append('text').classed('scaleText', true).attr('x', 50).attr('y', 50);
    svg.call(zoom);
    ...
    useEffect(() => {
        ...
        div.insert(() => svg.node(), ':first-child');
    }, []);
